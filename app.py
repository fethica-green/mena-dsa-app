...
def backup_excel(conn, backup_dir="backups"):
    """
    Crée chaque jour un fichier Excel de sauvegarde de la table 'records'
    dans backup_dir/travel_records_YYYY-MM-DD.xlsx, sans écraser si existant.
    """
    os.makedirs(backup_dir, exist_ok=True)
    today = date.today().isoformat()  # ex. '2025-05-15'
    dest = os.path.join(backup_dir, f"travel_records_{today}.xlsx")
    if os.path.exists(dest):
        return  # déjà sauvegardé aujourd'hui

    # Lire tous les enregistrements dans un DataFrame
...
def init_db(db_file="travel_records.db"):
    """Initialise la base SQLite et crée la table si nécessaire."""
...
def calculate_days(dep, ret):
    """Calcule (return−dep)+1 ou 1 si one-way."""
    try:
        if ret is None:
            return 1
        d = (ret - dep).days + 1
...
def to_excel(df: pd.DataFrame) -> bytes:
    """Exporte le DataFrame en Excel avec mise en forme."""
    output = io.BytesIO()
...
def main():
    ...
    with tab2:
        ...
        # Export full filtered table to Excel
        st.subheader("Export to Excel (all rows)")
        excel_all = to_excel(df)
        st.download_button(
            "Export Full Excel",
            data=excel_all,
            file_name="travel_records_full.xlsx",
        ...
        if selected_ids:
            ...
            st.download_button(
                ...
                data=excel_sel,
                file_name="travel_records_selected.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
    ...
    with tab3:
        ...
        st.bar_chart(
            df_dash.groupby('traveler')['co2_tons']
